...
В Laravel мы обращаемся по таким URL: ``http://localhost/users/1``. Сервер, разбивая строку обращения, передаёт наш запрос или обычной функции, или контроллеру.
...
2. Маршрут передаётся контроллеру или функции (пример последнего будет в конце документа).
...
2. Проверьте, что вы можете пользоваться утилитой artisan. Например, создайте через него любой контроллер.
