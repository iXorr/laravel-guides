...
Методы ``id`` и ``timestamps`` появляются по умолчанию при создании новых миграций. В ``id`` пишите название таблицы в единственном числе: так будет проще строить связи на уровне будущих моделей.

> Хорошая практика: называть вообще **каждый** атрибут таблицы, начиная с её названия в единственном числе. Но будет достаточно, если вы примените эту практику **только при создании ER-диаграммы**: например, не ``name``, а ``имя_пользователя``.
